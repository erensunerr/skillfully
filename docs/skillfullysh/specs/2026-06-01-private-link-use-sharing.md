...
- Store the setting on the skill as `anyoneWithLinkCanUse`.
- When enabled for a private skill, the unified `/api/skills/<skillId>/manifest`, `/files/...`, and `/install` routes serve the latest published release without bearer auth.

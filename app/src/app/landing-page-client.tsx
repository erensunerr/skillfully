...
function LandingVariantPending() {
  return (
    <main
      aria-hidden="true"
      className="min-h-dvh bg-[var(--paper)] text-[var(--ink)]"
      data-landing-variant-pending
    />
  );
}

function LandingVariantFrame({
  variant,
  children,
}: {
  ...
  return (
    <>
      ...
      {process.env.NODE_ENV === "development" ? (
        <DevABTestOverlay resolvedVariants={{ [AGENT_FIRST_EXPERIMENT_FLAG_KEY]: variant }} />
      ) : null}
    </>
  );
}
...
export default function LandingPageClient() {
  const variant = useFeatureFlagVariantKey(AGENT_FIRST_EXPERIMENT_FLAG_KEY);

  if (variant === undefined) {
    return (
      <LandingVariantFrame variant={variant}>
        <LandingVariantPending />
      </LandingVariantFrame>
    );
  }

  if (variant === AGENT_FIRST_VARIANT_FLAG_VALUE) {
    return (
      <LandingVariantFrame variant={variant}>
        <AgentFirstLanding />
      </LandingVariantFrame>

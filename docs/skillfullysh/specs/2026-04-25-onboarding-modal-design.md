...
- Add local interactions:
  ...
  - click `Read the guide` to navigate to `/docs`
...
- Add a minimal `/docs` guide page so the modal's `Read the guide` action does not navigate to a 404.
...
- Add static render coverage for the onboarding modal and `/docs` guide page.
- Keep existing dashboard view-state tests passing.
- Run the dashboard tests and production build.
- Browser-check the new `/docs` page on desktop and mobile. The authenticated dashboard modal requires a valid local `NEXT_PUBLIC_INSTANT_APP_ID` before it can be reached in-browser.
